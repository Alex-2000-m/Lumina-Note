...
Cascade 通过多层机制实现对代码仓库的感知：

#### 9.4.1 Workspace Layout（静态地图）

对话开始时，系统注入一个 `<workspace_layout>` 块，包含工作区的文件结构树：

```xml
<workspace_layout workspace="d:\Desktop\Lumina Note">
- .github/
  - workflows/
    - release.yml
- src/
...
- **压缩显示**，如 `[+23 files (23 md) & 0 dirs]`
- **只包含结构**，不包含文件内容

#### 9.4.2 动态探索工具

静态地图不够时，Cascade 使用工具动态获取信息：
...
#### 9.4.3 Memory System（跨会话记忆）

自动检索的记忆让 Cascade 在新对话中保持上下文：
...
│     └─ 系统级规则，始终适用                                  │
│                                                             │
│  2. User-provided Memories                                  │
│     └─ 用户显式提供的上下文                                  │
│                                                             │
│  3. System-retrieved Memories                               │
│     └─ 从历史对话自动检索，可能相关也可能不相关              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
#### 9.4.4 与 Aider Repo Map 对比

| 特性 | Windsurf | Aider |
|------|----------|-------|
| **结构快照** | ✅ workspace_layout | ✅ repo map |
| **代码骨架（函数签名）** | ❌ 无 | ✅ tree-sitter AST 解析 |
| **智能排序** | ❌ 无 | ✅ PageRank 算法 |
| **Token 预算控制** | ❌ 无 | ✅ 二分搜索动态调整 |
| **动态更新** | ❌ 会话内不更新 | ✅ 每次调用刷新 |
| **链接图分析** | ❌ 无 | ✅ 定义/引用追踪 |

**Aider Repo Map 的优势**：
- 使用 tree-sitter 解析代码 AST，提取函数签名、类定义
- 通过定义/引用关系构建代码图
- 使用 PageRank 算法对代码重要性排序
- 根据 Token 预算动态调整内容量

**Windsurf 的设计权衡**：
- 更简单的实现，依赖工具动态获取
- 不需要语言特定的 AST 解析器
- 通过 Fast Context 子代理弥补静态地图的不足
